...
function encrypt_SEED($str, $bszUser_key, $bszIV) {
    ...
    //return $bszIV."    ".json_encode($IVBytes);
    ...
    $ret = null;
    $bszChiperText = null;
    $pdwRoundKey = array_pad(array(), 128, 0);

    //return json_encode($keyBytes)."  ".json_encode($IVBytes)."  ".json_encode($planBytes)."  ".count($planBytes);

    KISA_SEED_CBC::SEED_CBC_Encrypt($keyBytes, $IVBytes, $planBytes, 0, count($planBytes));
    ...
    return base64_encode(implode("", Hex2String($ret)));
}
...
function decrypt_SEED($str, $bszUser_key, $bszIV) {
    ...
    return implode("", Hex2String($planBytresMessage));
}
