...
if (!defined('_KISA_COMMON_LIB')) {
    ...
    class Common {
        ...
        static function arraycopy_system(&$src, $src_offset, &$dst, $dst_offset, $length) {
            //return json_encode($src)." 1 ".$src_offset."   ".json_encode($dst)."  ".$dst_offset."  ".$length;
            for ($i = 0; $i < $length; $i++) {
                if(isset($src[$src_offset + $i])) {
                    $dst[$dst_offset + $i] = $src[$src_offset + $i];
                } else {
                    $dst[$dst_offset + $i] = null;
                }
            }
            //return json_encode($src)."  ".$src_offset."   ".json_encode($dst)."  ".$dst_offset."  ".$length;
        }
...
class KISA_SEED_CBC {
    ...
    static function SEED_CBC_Process(&$pInfo, &$in, $inLen, &$out, &$outLen) {
        ...
        if (KISA_ENC_DEC::KISA_ENCRYPT == $pInfo->encrypt) {
            ...
            //return json_encode($in)." 1 ".$in_offset."   ".json_encode($pInfo->cbc_buffer)."  ".$pInfo->buffer_length;
    ...
    static function SEED_CBC_Encrypt(&$pbszUserKey, &$pbszIV, &$message, $message_offset, $message_length) {
        ...
        //return json_encode($info)."  ".json_encode($pbszUserKey)."  ".json_encode($pbszIV);
        ...
        //return json_encode($info)."  ".json_encode($data)."  ".$nPlainTextLen."  ".json_encode($outbuf)."  ".$nRetOutLeng;
