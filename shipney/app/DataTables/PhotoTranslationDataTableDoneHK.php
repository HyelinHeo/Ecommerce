<?php
/**
 * File name: PhotoTranslationDataTableDoneHK.php
...
class PhotoTranslationDataTableDoneHK extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        ...
        $dataTable = $dataTable
            ...
            ->editColumn('id', function ($order) {
                return getRouteColumn($order, 'id', 'photoTranslationsHK.show');
            })
    ...
    public function query(PhotoTranslation $model)
    {
        ...
        ->where(function ($query) {
            $query->whereNotNull('address_photo1')
                    ->where('nation_code','=','HK')
                    ->where(function ($querys) {
                        $querys->where('address_trans_done', '=', 'complete');
                    });
        })
        ->where(function ($query) {
            $query->whereNotNull('receiver_name_photo1')
                    ->where('nation_code','=','HK')
                    ->where(function ($querys) {
                        $querys->where('receiver_name_trans_done', '=', 'complete');
                    });
        })
        ->orderBy('id', 'desc');
    }
